...
This chapter aims to analyze and propose solutions for subtle, but relevant, problems in the overnight curve calibration. The focus will be mainly on the EUR market and, in particular, on the short-term section.
The first issue examined is related to the inclusion of forward start instruments (as the ECB OIS) that leads to oscillation caused by overlapping instruments: to solve the problem, this work suggest to build a meta-instrument called "{\it Forward Stub}" to cover the section between the maturity of the last spot starting OIS and the settlement of the first ECB OIS which implied value can be derived assuming a no-arbitrage condition. To support this solution, a re-pricing error analysis will be presented in order to show that this approach can minimize distortion. This re-pricing analysis will be exploited also in the other issues and consists in using both curves (with and without the solution) to price a wide subset of all ICAP (see \cite{icap}) overnight indexed instruments. The purpose is to demonstrate that using the solutions proposed in this work the ICAP instruments re-pricing will be more accurate which means that the outcome curve quality as been improved. All the analysis will be performed by mean of QuantLibXL 1.8 version \footnote{QuantLib is a C++ free/open-source library for quantitative finance, for more information see \cite{quantlib}}.

The next issue is the empirical evidence that the forward overnight rates are generally constant between ECB monetary policy board meeting dates: because of this, a log-linear discount interpolation would be a good fit. Otherwise, flat forward rates might be reasonable up to the end of the ECB OIS strip (usually 2 years) but hardly realistic after that. This is the rationale to suggest the use of a "{\it Mixed Interpolation}" which merges the two different interpolation regimes for the short and long term of the overnight rate curve. Finally the results of the mixed interpolation applied on the curve will be presented taking advantage of the same re-pricing analysis explained before.

The next issue is how to deal with jumps, estimating their positive or negative sizes. Finally the approximation necessary to include {\it Arithmetic Average OIS} \footnote{Arithmetic FF-OIS as well as the Mixed Interpolation are implemented in latest QuantLib version and exported in QuantLibXL} will be described, as relevant for the USD overnight curve bootstrap. \\
To perform analysis a EUR market dataset as of January 29, 2016 (spot date February 2, 2016) is used \ref{fig:dataset}. Using all market instruments shown in Figure \ref{fig:dataset} for curve bootstrapping might lead to distorsive overfitting: that is why only the most liquid subset of quotes is used. While instruments included in the bootstrapping are perfectly re-priced by construction, the rest will have re-pricing errors. The validation of this work's analysis will mostly consist in the minimization of the excluded instruments re-pricing errors, while obtaining good properties for the overall curve.
...
This chapter faces 3 advanced overnight curve problems and presents some possible solutions. First of all, this work proposes to build a forward Stub in order to fix the overlapping instruments problem caused by the not perfect concatenation between spot starting OIS and ECB dated OIS. The results of this solution have been presented taking advantage of a repricing errors analysis which shows that excluding the overlapping instrument and including the forward Stub improve the curve's quality minimizing all the out-of-curve instruments errors.

Moving ahead to the next point, the mixed interpolation has been presented as a particular interpolator which merge two different techniques. In order to describe the overnight curve behaviour in the most accurate way, this work proposes to mix a linear interpolation on log-discounts for the short term curve's section with a monotone cubic natural spline for the mid-long term section. Using the same repricing approach proposed in the previous issue it has been shown that this configuration leads to a very good curve's fit which is better than the one obtained interpolating the whole curve with a log-linear interpolation or a monotone cubic natural spline interpolation. 

Finally the jumps problem for the EUR market and the USD market has been shown. Following the best practice proposed by Ametrano-Mazzocchi (\cite{amemaz}) based on the  Burghardt approach (\cite{burgh}) this work proposed a simple way to estimate negative jumps as the ones visible in the Fed Fund time series.  
...
\bibitem{opengamma} {\em Interest rate instruments and market conventions guide}, Quantitative research, OpenGamma, December 2013.\\
