...
\begin{description}
    ...
    \item{-} Jumps and Turn of Year \textit{(TOY)} effect in foreign currencies curves (the USD problem will be presented);
...
This chapter wants to focus on overnight curve construction passing through every issue outlined in Chapter \ref{chap:1}. The objective is to present a calibration best practice related on the specific overnight curve case which is the core argument of this work.
    ...
    This particular overnight behaviour (piece-wise constant in short region and smooth in long region) can be fixed using two different kinds of interpolations: linear on log discount and monotonic cubic natural spline on log discount \footnote{the characteristic of this mixed interpolation will be treated in \ref{section:3.2}.}. The linear one is typically used for pricing purpose in the short term section of the curve (for example to obtain the quotation of $OIS$ up to 1-2 years) and the cubic one for discounting or long term pricing. \\
    Figure 
...
This chapter aims to analyze and propose solutions to some advanced problems related to the overnight curve calibration. First of all, the attention will be placed on the EUR market and, in particular, on the short-term section where the inclusion of forward start instruments (as the ECB OIS) leads to wide oscillation caused by overlapping instruments. To solve the problem, this work suggest to build a forward meta-instrument called "{\it Stub}" to cover the section between the maturity of the last spot starting OIS and the settlement of the first ECB OIS which implied value can be derived assuming a no-arbitrage condition. To support this solution, a re-pricing error analysis will be presented in order to show that this approach can minimize distortion. This re-pricing analysis will be exploited also in the other issues and consists in using both curves (with and without the solution) to price a wide subset of all ICAP (see \cite{icap}) overnight indexed instruments. The purpose is to demonstrate that using the solutions proposed in this work the ICAP instruments re-pricing will be more accurate which means that the outcome curve quality as been improved. All the analysis will be performed by mean of QuantLibXL 1.8 version \footnote{QuantLib is a C++ free/open-source library for quantitative finance, for more information see \cite{quantlib}}. \\
The next step will be focused on the overnight curve behaviour. Since empirical evidence shows that the short term section tends to be linear between European Central Bank monetary policy board meeting dates, it is straightforward that, in this context, a log-linear interpolation fits well the curve behaviour. Otherwise, The flat interpolator is consistent till the end of the ECB OIS strip but, after that, assuming constant forward rates becomes quite improbable due to the fact that it is not possible to estimate the ECB monetary policy on a long time frame\footnote{or maybe, in two year, the European Central Bank might not exist anymore}. This is the reason why this work suggest to calibrate the overnight curve by means of a so called: "{\it Mixed Interpolation}" which merges two different interpolation like, for example, a log-linear on discount factors up to a specific date and a cubic spline after it. Finally the results of the mixed interpolation applied on the curve will be presented taking advantage of the same re-pricing analysis explained before.\\
Moving ahead to the next issue, the work will be extended to both EUR and USD overnight curve. The jumps problem will be treated in order to present a methodology to estimate their size following a similar approach as the one proposed by Burghardt (\cite{burgh}) and recovered by Ametrano-Mazzocchi (\cite{amemaz}) for the EUR market case. This approach will be adapted in order to account also negative jumps, as the one shown in the Fed Funds time series. \\
Focusing just on the USD market case, the {\it Arithmetic Average Fed Funds OIS} \footnote{Arithmetic FF-OIS as well as the Mixed Interpolation are implemented in latest QuantLib version and exported in QuantLibXL} will be briefly described stressing on the pros and cons of their inclusion in the USD overnight curve bootstrap. \\
To perform all the EUR market analysis a dataset dated on {\it 29-January-2016} will be used, which is shown in the following figure. 
...
\newpage
In order to avoid overfitting distorsion, all overnight curves are bootstrapped starting from a subset of all market quotes shown in Figure \ref{fig:dataset} and using a $(t+2)$ spot date as the settlement date. \\
It is useful to remember that, while an included instruments is always re-priced perfectly, all the others are interpolated and, as a consequence, will show re-pricing errors. The validation of this work's analysis will pass through the proof that the solutions proposed permits to minimize the out-of-curve instruments re-pricing errors. The overnight curve composition is visible in the next figure.

\begin{figure}[!h]
\centering
\includegraphics[width=0.9\textwidth]{images/subset.png}
\caption{All the instruments included in the EUR overnight curve calibration.}
\label{fig:subset}
\end{figure}
    
    \newpage
    \section{Forward Stub} \label{section:3.2}
    Starting from the first point, a particular overnight curve problem due to the manage of both spot starting instruments (like OIS) and forward start instruments (like ECB OIS) in the bootstrap algorithm will be presented. The problem occurs since spot starting OIS maturity dates, in most of the cases, does not coincide with the settlement date of the first ECB OIS and this leads to an overlapping instruments problem. Since the calibration algorithm must re-price perfectly all the included market quotes, the overlapping instruments force the calibrator to re-price both of them and, in order to respect this constrain, it will introduce wide oscillation in the curve shape. \\ 
    To solve this problem different approach can be followed:
    
     \begin{itemize}
         \item{} A possibility is to include all spot starting OIS up to the first one overlapping with the first forward start ECB OIS (except for the lucky case in which a spot starting OIS maturity coincides with the ECB dates OIS settlement but this situation can only happens one day for each instrument). This solution's benefit is that the ON curve will be totally covered because the bootstrap algorithm can manage a sequence of maturities which permits a "pillar-to-pillar" standard bootstrap. Otherwise this choice implies that there will be a short time-frame presenting overlapping instruments as visible in Figure \ref{fig:overlapping}. \\
         
        \begin{figure}[!h]
        \centering
        \includegraphics[width=0.8\textwidth]{images/overlappingsol.png}
        ...
        \begin{figure}[!h]
        \centering
        \includegraphics[width=0.8\textwidth]{images/excludingsol.png}
        \caption{The excluding solution using 2 February 2016 values.}
        \label{fig:exclude}
        \end{figure}
        
        This answers are not satisfactory and seems that there is no solution taking advantage of the only market instruments. This work's suggestion is to create a forward "Meta-Instrument", from now called {\it Stub}, which value can be derived from available market quotes.
        \end{itemize}
        
        \newpage
        \subsection{Implied value calculation} \label{subsection:3.2.1}
        Let's suppose now to be in the situation described in Figure \ref{fig:exclude} in which there is a not covered curve's section. The Stub quote represents a forward rate which starts from the last not-overlapping instrument's (the OIS1M) maturity date and ends in the first ECB Dated settlement date.\\
        
        \begin{figure}[!h]
        \centering
        \includegraphics[width=0.8\textwidth]{images/excludingsol2.png}
        \label{fig:excluded2}
        \end{figure}
        ...
        \label{fig:stub}
        \end{figure}
        As visible, no-arbitrage condition implies that investing in OIS1M ({\it first segment}), Stub ({\it second segment}) and in ECB OIS till the OIS2M maturity ({\it last segment}) must be equal to investing in OIS2M ({\it dotted segment}). In this situation, the Stub implied value can be calculated as:\\
        ...
        $$ \mbox{{\it Stub}} = \frac{\left[\frac{(1+F(0,t\ped{3}) \cdot \tau(0,t\ped{3}))}{(1+F(0,t\ped{1}) \cdot \tau(0,t\ped{1})) \cdot (1+F(t\ped{2},t\ped{3}) \cdot \tau(t\ped{2},t\ped{3}))}-1\right]}{\tau(t\ped{1},t\ped{2})}$$\\
        Calibrating the overnight curve using this forward Stub improve consistently the curve quality avoiding unwanted distortion and oscillation as it will be shown in next section. Otherwise, this solution can be implemented only if the bootstrap algorithm uses a log-linear interpolation for the short-term section of the curve. Interpolating log-linearly the overnight curve becomes piecewise constant and this assumption permits to derive the $e^{\int_{t\ped{2}}^{t\ped{3}} f_{inst}ds}$ value that must be equal to ECB OIS market quote. The good news is that, as it will be shown in the next section, the best practise to interpolate the ON curve is by means of a mixed interpolation, which interpolates log-linearly the short term section of the curve. \\
        It is also important to underlying that the Stub algorithm produces good results also in limit cases as, for example, when $\tau(t\ped{1},t\ped{2}) \longrightarrow \frac{1}{365}$ that represents the particular case in which the Stub quote duration is just 1 day. Finally, during a year, there surely happen a particular scenario in which, since the first ECB OIS fixing becomes earlier and earlier, the discarded spot instruments is the first spot starting
        OIS. This case implies that the so called "forward Stub" it is not forward start anymore because it becomes a spot instrument which covers the year fraction: $\tau(0,t\ped{1})$ (where $t\ped{1}$ is the first ECB OIS fixing date). In this case, the "spot stub" implied value is:\\
        
        $$ \mbox{{\it Stub}} = \frac{\left[\frac{1+F(0,t\ped{2}) \cdot \tau(0,t\ped{2})}{1+F(t\ped{1},t\ped{2}) \cdot \tau(t\ped{1},t\ped{2})}-1\right]}{\tau(t\ped{0},t\ped{1})}$$
        ...
        \begin{figure}[!h]
        \centering
        \includegraphics[width=0.8\textwidth]{images/excludingcurve.png}
        \caption{ON Curve excluding overlapping instrument (Evaluation date 2 February 2016).}
        \label{fig:excludingcurve}
        \end{figure}
        
        \newpage
        This solution's big problem is that the bootstrap algorithm extrapolate backward from ECB Dated market quote the non-covered part of the curve leading to high distortions especially when preceding instrument and ECB OIS quotes are distant.\\
        ...
        \begin{figure}[!h]
        \centering
        \includegraphics[width=0.8\textwidth]{images/includingcurve.png}
        \caption{ON Curve including overlapping instrument (Evaluation date 2 February 2016).}
        \label{fig:includingcurve}
        \end{figure}
        This option produces distortions due to the fact that the calibration will introduce strange oscillations in order to reprice the overlapping instrument. In fact, the Figure \ref{fig:includingcurve}, shows how the curve is pushed down till $-0.40\%$ and then turns back to the ECB OIS level. It is quite obvious that this strange curve's behaviour is not due to market quotes but to calibration's discrepancy resulting from the overlapping section. Finally, let's present the forward Stub solution:
        ...
        \newpage
        As visible, there are no wide oscillation in the curve's shape and the repricing errors analysis, represented in Figure \ref{fig:repricingimage} and \ref{fig:errors} , shows that this approach is the one which minimize all out-of-curve instruments, ensuring also the perfect reprice of the overlapping instrument excluded. Obviously, all the included instruments are always perfectly repriced.\\
        
        \begin{figure}[!h]
        \centering
        \includegraphics[width=0.65\textwidth]{images/repricingimage.png}
        ...
        \begin{figure}[!h]
        \centering
        \includegraphics[width=0.8\textwidth]{images/errors.png}
    ...
    \section{Mixed Interpolation}\label{section:3.2}
        Moving to the next point, the {\itshape Mixed Interpolation} has already been introduced specifying that it merges two different kind of interpolation. Since the overnight curve time series shows a piecewise constant behaviour in the short term section, the best way to replicate the curve's shape is by means of a linear interpolation on discount factor which provides the stepped overnight curve shown in the next figure.
        
        \begin{figure}[!h]
        \centering
        \includegraphics[width=0.59\textwidth]{images/shortlinear.png}
        \caption{Piecewise flat forward rates obtained through linear interpolation on log-discounts.}
        \label{fig:shortlinear}
        \end{figure}
        
       After the ECB dated OIS strip, however, assuming a piecewise constant curve is no more reasonable as there are no available information about the ECB monetary policy. For this reason, the flat interpolation on the mid/long term curve section leads to big inconsistency and so it becomes better to use an interpolation technique which produces smooth forward rates as the Hyman filtered monotonic cubic natural spline on log-discounts shown in the following figure. 
       
       \begin{figure}[!h]
       \centering
       \includegraphics[width=0.59\textwidth]{images/shorthyman.png}
       \caption{Monotonic Cubic Natural Spline interpolation on log-discount factors.}
       \label{fig:shorthyman}
       \end{figure}
       
       Since using a log-linear or a monotonic cubic natural spline interpolation for the whole curve leads, in both cases, to distortions, this work suggest that the best way to model the overnight curve behaviour is to merge this two techniques leading to a mixed interpolator. The mixed interpolation is implemented in QuantLib 1.8 version in two different ways:
        ...
        \includegraphics[width=\textwidth]{images/splitrange.png}
        \label{splitrange}
        \end{subfigure}
        \begin{subfigure}[b]{0.62\textwidth}
        \includegraphics[width=\textwidth]{images/sharerange.png}
        \label{sharerange}
        \end{subfigure}
        \caption{Split Range and Share Range approach merging linear and monotonic cubic natural spline interpolation on log-discounts.}
        \label{merge}
        \end{figure}
    
        As visible in the previous figures the two approaches produce an almost identical behaviour that makes it hard to understand which is the best.\\
        Finally, the table below summarize the repricing error analysis results. \\
        
        \begin{figure}[!h]
        \centering
        \includegraphics[width=0.8\textwidth]{images/repricingmixed.png}
        \caption{Repricing error analysis using a linear interpolation for the whole curve (first column), a monotonic cubic spline for the whole curve (second column), a share range mixed interpolation (third column) and a split range mixed interpolation (last column).}
        \label{fig:repricingmixed}
        \end{figure}
        
        Analyzing the repricing table above it is possible to draw some conclusions. First of all, looking to the not included instruments, it is visible that the only approach that leads to higher errors is the Hyman Monotone Cubic one which fails to repricing correctly the 2-months maturity OIS (which is the instrument excluded in favor of the Stub). As already explained in the previous section the Stub implied quote is derived imposing a no-arbitrage condition and assuming a flat interpolation. Since the other approaches are interpolated linearly in that section they re-price that instrument perfectly but this is not the Cubic Spline case, which is not flat, and leads to a relevant error of 0.97. This result means that the Hyman Monotone Cubic approach does not permits the introduction of the forward stub and this can introduce high distortion in the calibration which are avoided using a piecewise constant interpolator (as the other apporaches). \\
        Moving ahead, the other instruments are included in the calibration and they must be repriced perfectly. Otherwise, in order to test the calibration goodness on the mid/long tenor section, the bootstrap has been implemented excluding, one-by-one, the OIS5Y, OIS6Y, OIS10Y and the OIS11Y and then repricing them (the errors are outlined in red). Doing this, it is possible to understand how the different approaches are able to reprice instruments on the mid/long curve section. Analyzing the results, it is straightforward that the whole linear interpolated curve is not able to do a good reprice of this instruments especially in the mid section (5Y-6Y) where the sampled curve was very steep. \\
        Summing up, the repricing analysis shows that doing a linear or a cubic interpolation for the whole curve leads to relevant distortion while the mixed interpolation approaches are able to merge the benefits of both allowing the forward Stub inclusion but also ensuring a very good reprice of all mid/long tenor OIS. Otherwise, the work can not outline which mixed approach (Split Range or Share Range) is the best one since they lead to almost identical results. 
        
        \begin{figure}[!h]
        \centering
        \includegraphics[width=0.75\textwidth]{images/mixedcurve.png}
        \caption{Mixed Interpolated 60 years curve.}
        \label{fig:mixedcurve}
        \end{figure}
        
    \section{Jump estimation} \label{section:3.3}
    As already discussed in previous chapters, the key point in a state-of-the-art bootstrapping is to obtain smooth forward rates.
    In order to do that, for even the best interpolation scheme to be effective, any market jump must be removed before the curve calibration and than added back at the end of the process.
    To perform the analysis for the EUR market case, the same dataset has been used while, obviously, for the USD another dataset, dated on {\it 4-October-2016}, has been used which is shown in the following figure.
    ...
    As the EUR market case, to perform the USD overnight curve bootstrap a wide subset of the instruments listed above will be used which is summarized in the next figure.
    
    \begin{figure}[!h]
    \centering
    \includegraphics[width=1.0\textwidth]{images/usdoncurve.png}
    \caption{All the instruments included in the USD overnight curve calibration.}
    \label{fig:usdoncurve}
    \end{figure}
    
       \subsection{The EUR market case: Positive Jumps} \label{subsection:3.3.2}
       A rate jump can be seen, in a financial point of view, as a higher index fixing due to increased search of liquidity of market participants caused by end-of-month or end-of-year capital requirements. This definition is good for the EUR case in which market evidence shows positive jumps as visible in the figure below.
       
       \begin{figure}[!h]
       \centering
       \includegraphics[width=0.55\textwidth]{images/turnofyear.png}
       \caption{December 2014 turn of year effect shown on Bloomberg terminal.}
       \label{fig:turnofyear}
       \end{figure}
       However, this definition is not consistent for the U.S. case in which market evidence shows negative jumps and so American market participants tend to have a decreasing necessity of liquidity every end-of-month. The most relevant rate jump is the so called {\it Turn-Of-Year} (TOY) effect observable in the last working day in market quotations spanning across the end of the year. Other Euribor indexes with longer tenor display smaller jumps when their maturity crosses the same border; in fact jumps amplitude decrease as the rate tenor increase.\\
       ...
       \begin{enumerate}
        ...
        \end{enumerate}
        The Figure \ref{fig:bloombergeur} shows the past Eonia fixings for the last year. As visible it occurs a jump at least at every end of month. To obtain a smooth forwarding curve, this particular behaviour must be addressed in the calibration taking care to clear the entire curve from jumps before starting the bootstrap.
        
        \begin{figure}[!h]
        \centering
        \includegraphics[width=0.65\textwidth]{images/bloombergeur.png}
        \caption{Eonia fixings time series on Bloomberg terminal.}
        \label{fig:bloombergeur}
        \end{figure}
         
        \newpage
        The next to Figures shows the impact of first and second jump inclusion in the curves shape. \\
        
        \begin{figure}[!h]
        \centering
        \includegraphics[width=0.85\textwidth]{images/firstjumpeur1.png}
        ...
        \begin{figure}[!h]
        \centering
        \includegraphics[width=0.85\textwidth]{images/firstjumpeur2.png}
        \caption{The same curve in Figure \ref{fig:firstjumpeur1} including the first positive jump.}
        \label{fig:firstjumpeur2}
        \end{figure}
        
        \begin{figure}[!h]
        \centering
        \includegraphics[width=0.85\textwidth]{images/firstjumpeur3.png}
        \caption{The same curve in Figure \ref{fig:firstjumpeur1} including first and second jump.}
        \label{fig:firstjumpeur3}
        \end{figure}
        \newpage
        As visible, the short term section of the curve is changed due the fact that the first jumps has been accounted. To preserve the value of the instantaneous forward rate integral in that section, the calibration algorithm pushes down all the segment changing the curve shape.
        
        \subsection{The USD market case: Negative Jumps}
        Moving to the USD market, the situation looks inverted. In fact, as visible in Figure \ref{fig:bloombergusd}, the Fed Funds fixings shows negative jumps every end-of-month and with a particular deep size. In a financial point of view this is due to the excess of liquidity that market participants hold and want to sell at the end of each month. This behaviour tends to be constant during time, in fact, in the last two years, the Fed Funds time series shows negative jumps every end-of-month with no exceptions and this leads to a good predictability of this phenomenon.\\  
        
        \begin{figure}[!h]
        \centering
        \includegraphics[width=0.75\textwidth]{images/bloombergusd.png}
        \caption{Fed Funds fixings time series on Bloomberg terminal.}
        \label{fig:bloombergusd}
        \end{figure}
        To estimate this kind of jumps, however, the Ametrano-Mazzocchi approach for the EUR market case must be reviewed so that it can account negative sizes. In particular the formula at point 2 becames:
        
        $$\left[F^{Interp}(t\ped{1},t\ped{2}) - F^{Original}(t\ped{1},t\ped{2})\right] \cdot \tau(t\ped{1},t\ped{2}) = J^{Size}*\tau^{J}$$\\
        Applying the same procedure but using the reviewed formula it is now possible to make an estimation of the USD jumps and account them in the USD overnight curve as the next figures show.
        
        \begin{figure}[!h]
        \centering
        \includegraphics[width=0.85\textwidth]{images/firstjumpusd1.png}
        \caption{First two year section of the USD overnight curve without jumps..}
        \label{fig:firstjumpusd1}
        \end{figure}
    
        \begin{figure}[!h]
        \centering
        \includegraphics[width=0.85\textwidth]{images/firstjumpusd2.png}
        \caption{The same curve in Figure \ref{fig:firstjumpusd1} including the first negative jump.}
        \label{fig:firstjumpusd2}
        \end{figure}
        
        \begin{figure}[!h]
        \centering
        \includegraphics[width=0.85\textwidth]{images/firstjumpusd3.png}
        \caption{The same curve in Figure \ref{fig:firstjumpusd1} including first and second jump.}
        \label{fig:firstjumpusd3}
        \end{figure}
        
        \newpage
        It is clear that, since the piecewise flat steps of the curve are given by market quotes which are the integral of instantaneous forward rates between an interval $(t\ped{1},t\ped{2})$, if this integral must account a deep below level point, the residual part is shifted up in order to maintain the integral value equal.\\

    \section{Fed Funds Arithmetic Average OIS} \label{section:3.4}
    From the previous section the subject have been moved to U.S. Dollar overnight yield curve calibration. This part aims to discuss how the USD overnight (USDON) curve can be improved including in the bootstrap the Fed Funds (FF) arithmetic OIS instead of the simple OIS indexed to the Fed Funds rate. This is a recommendable practice due to the fact that arithmetic OIS are traded in the dealer's market more actively and so, fore the liquidity criteria discussed in \ref{section:2.1}, it is better to include them in the calibration algorithm.
    
        \subsection{Fed Funds OIS Vs. Arithmetic-LIBOR basis swap} \label{subsection:3.4.1}
        FF-OIS, as the EUR-OIS already discussed in \ref{subsection:2.1.2}, are particular IRS which floating-leg pays the daily compounded FF rate. In a interest rate basis swap (IRBS), instead, two different floating rates in the same currency are exchanged which can be, for example, two LIBORs with different tenors. Arithmetic-LIBORs, so, are basis swaps which exchange the FF rate on a leg and a LIBOR rate on the other. In derivative products like swaps the FF rates, as the EUR overnight ones, are never paid on a daily basis but paid collectively in a single coupon. The difference between FF-OIS and Arithmetic-LIBOR basis swaps is that, while in FF-OIS coupons are done on a daily compounded basis, in Arithmetic-LIBOR basis swaps are built doing an arithmetic average. This calculation difference leads to some complications which implies two convexity adjustment which derivation is out of this work objects and will be briefly introduced later.\\
        ...
        $$D(t,T) = E^{M}\ped{t} \left[e^{-\int_t^T \bar{r}(u)du}\right]$$
        This permits to express the value $V(t,T)$ of a collateralized FF-OIS over the accrual period $(t\ped{1},t\ped{2})$ as:
        
        \begin{equation*}
        \begin{split}
        V^{OIS}(t\ped{1},t\ped{2}) &= E^M \left[e^{-\int_0^{t\ped{2}} \bar{r}(u)du} \tau(t\ped{1},t\ped{2})R^{OIS}(t\ped{1},t\ped{2})\right] \\
        &= E^M \left[e^{-\int_0^{t\ped{2}} \bar{r}(u)du} \left[\prod_{k=1}^{K} (1+\tau\ped{k}r\ped{k})-1\right]\right] \\
        ...
        \begin{equation*}
        \begin{split}
        V^{Arithm}(t\ped{1},t\ped{2}) &= E^M \left[e^{-\int_0^{t\ped{2}} \bar{r}(u)du} \tau(t\ped{1},t\ped{2})R^{Arithm}(t\ped{1},t\ped{2})\right] \\
        &= E^M \left[e^{-\int_0^{t\ped{2}} \bar{r}(u)du} \left[\prod_{k=1}^{K} \tau\ped{k}r\ped{k}\right]\right] \\
        &\approx E^M \left[e^{-\int_0^{t\ped{2}} r(u)du} \int_{t\ped{1}}^{t\ped{2}} \bar{r}(u)du\right] \\
        &= D(0,t\ped{2})E^{t\ped2}\left[\int_{t\ped{1}}^{t\ped{2}} \bar{r}(u)du\right]
        \end{split}
        \end{equation*}
        Where the collateralized Bond value $D(0,t\ped{2})$ with maturity $t\ped{2}$ is the numeraire which permits to move in a forward risk neutral world and $e^{-\int_0^{t\ped{2}} \bar{r}(u)du}$ is the stochastic discount factor.
        ...
        \begin{figure}[!h]
        \centering
        \includegraphics[width=0.8\textwidth]{images/convexity2.png}
        ...
        $$ConvAdj_2 = \frac{1}{\tau(t\ped{1},t\ped{2})}log(1+\tau(t\ped{1},t\ped{2})*R_f^{OIS}(0;t\ped{1},t\ped{2})) - R_f^{Arithm}(0;t\ped{1},t\ped{2})$$
        ...
        \newpage
        The Arithmetic basis swaps does not exist for tenor less than one year; in fact the impact on the curve is visible from one year after the settlement date and consists in a smoother shape.
...
\bibitem{amebianc2} F.M.~Ametrano, M. ~Bianchetti, {\em Bootstrapping the illiquidity, multiple yield curves construction for market coherent forward rates estimation}, SSRN, March 2009.\\
