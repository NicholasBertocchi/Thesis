...
\end{figure}
    ...
    \begin{figure}[!h]
    \centering
    \includegraphics[width=0.7\textwidth]{images/overlappingsol.png}
    \caption{The overlapping solution scenario.}
    \label{fig:overlapping}
    \end{figure}
    Since the calibration algorithm must re-price perfectly all the included market quotes, the overlapping instrument force the calibrator to re-price two times the section covered by two instruments and, in order to respect this constrain, it will introduce oscillation in the curve shape as shown in figure \ref{fig:includingcurve}.
    \begin{figure}[!h]
    \centering
    \includegraphics[width=0.7\textwidth]{images/includingcurve.png}
    \caption{ON Curve excluding overlapping instrument.}
    \label{fig:includingcurve}
    \end{figure}
    This scenario produces distortions due to the fact that the calibration will introduce strange oscillations in order to reprice the overlapping instrument. In fact, the Figure \ref{fig:includingcurve}, shows how the curve is pushed down till $-0.40\%$ and then turns back to the ECB OIS level. It is quite obvious that this strange curve's behaviour is not due to market quotes but to calibration's discrepancy resulting from the overlapping section. To solve this problem different approach can be followed:
     \begin{itemize}
         \item{} A possibility is to include all spot starting OIS up to the first one overlapping with the first forward start ECB OIS (except for the lucky case in which a spot starting OIS maturity coincides with the ECB dates OIS settlement but this situation can only happens one day for each instrument). This solution's benefit is that the ON curve will be totally covered because the bootstrap algorithm can manage a sequence of maturities which permits a "pillar-to-pillar" standard bootstrap. Otherwise this choice implies that there will be a short time-frame presenting overlapping instruments visible in previous examples which shows that OIS with 2-month maturity ({\it 4 April 2016}) overlaps the first ECB OIS with settlement ({\it 16 March 2016}). The overlap section cause oscillations in the curve's shape and, as a consequence, an impact on not included instruments repricing errors. This errors are also deeply connected to the first section curve's shape; in fact this solution may leads to negligible repricing errors only if spot starting OIS and first ECB OIS presents close values (which means that the first curve section tends to be flat). Otherwise, empirical evidence shows that, sometimes, market idiosyncrasies can affect first section curve's shape making it highly downward/upward sloping (as the Figure \ref{fig:overlapping} suggest). In this particular cases also the overlapping solution fails leading to relevant errors.
        ...
        \begin{figure}[!h]
        \centering
        \includegraphics[width=0.7\textwidth]{images/excludingsol.png}
        \caption{The excluding solution scenario.}
        \label{fig:exclude}
        \end{figure}
        The impact on the shape is shown in figure \ref{fig:excludingcurve} and is visible how the calibrator extrapolates backward from ECB Dated market quote the non-covered part of the curve leading to high distortions especially when preceding instrument and ECB OIS quotes are distant. 
        \begin{figure}[!h]
        \centering
        ...
        \end{figure}
        This solutions are not satisfactory and seems that there is no solution taking advantage of the only market instruments. This work's suggestion is to create a forward "Meta-Instrument", from now called {\it Forward Stub}, which cover the missing section and which value can be derived from available market quotes.
        \end{itemize}
        Let's suppose now to be in the situation described in Figure \ref{fig:exclude} in which there is a not covered curve's section. The Stub quote represents the forward rate which starts from the last not-overlapping instrument's (the OIS1M) maturity date and ends in the first ECB dated settlement date.
        \begin{figure}[!h]
        \centering
        ...
        \label{fig:stub}
        \end{figure}
        The Stub forward rate is implied in the market and can be derived assuming a no-arbitrage condition and assuming that forward rates from $t\ped{2}$ and $t\ped{4}$ are constant, where:\\
        \\
        $t\ped{1} = \mbox{Last non-overlapping instrument's maturity}$ \\
        $t\ped{2} = \mbox{First ECB dated OIS settlement date}$ \\
        $t\ped{3} = \mbox{Overlapping instrument's maturity date}$ \\  
        $t\ped{4} = \mbox{First ECB dated OIS maturity date}$ \\
        $f(s) = \mbox{Instantaneous forward rate}$\\
        \\
        In our case:
        \\
        $t\ped{1} = \mbox{March 2, 2016}$ \\
        $t\ped{2} = \mbox{March 16, 2016}$ \\
        $t\ped{3} = \mbox{April 4, 2016}$ \\  
        $t\ped{4} = \mbox{April 27, 2016}$ \\
        \\
        That is consistent with the empirical market evidence which shows flat forward rates between ECB monetary policy dates. In order to not introduce arbitrages, it is necessary to obtain the Stub quote imposing that the excluded instrument (OIS2M in the example) must be repriced perfectly. In order to replicate the OIS2M value the following relationship must hold:
        
        $$\int_{0}^{t\ped{1}} f(s)ds \cdot \int_{t\ped{1}}^{t\ped{2}} f(s)ds \cdot \int_{t\ped{2}}^{t\ped{3}} f(s)ds = \int_{0}^{t\ped{3}} f(s)ds$$
        As visible, no-arbitrage condition implies that investing in OIS1M ({\it first segment}), Forward Stub ({\it second segment}) and in ECB OIS till the OIS2M maturity ({\it last segment}) must be equal to investing in OIS2M ({\it dotted segment}). In this situation, the Stub implied value can be calculated as:\\
        ...
        Calibrating the overnight curve using this forward Stub improve consistently the curve quality avoiding unwanted distortion and oscillation as it will be shown in next section. Otherwise, this solution can be implemented only if the bootstrap algorithm uses a linear interpolation on log-discounts for the short-term section of the curve. Interpolating log-linearly the overnight curve becomes piecewise constant and this assumption permits to derive the $\int_{t\ped{2}}^{t\ped{3}} f(s)ds$ value that must be equal to ECB OIS market quote. The good news is that, as it will be shown in the next section, the best practise to interpolate the EURON curve is by means of a mixed interpolation, which interpolates log-linearly the short term section of the curve. \\
        ...
        Furthermore, during a year, there surely happen a particular scenario in which, since the first ECB OIS fixing becomes earlier and earlier, the discarded spot instruments is the spot week (SW) spot starting OIS. This case implies that the so called "Forward Stub" it is not forward start anymore because it becomes a spot instrument which covers the year fraction: $\tau(0,t\ped{1})$ (where $t\ped{1}$ is the first ECB OIS fixing date). In this case, the "spot stub" implied value is:
        ...
        The impact on the curve's shape of the forward stub solution in presented in the following figure. It is visible that this approach avoid strange behaviour and 
        ...
        Finally, the Figure \ref{fig:errors} shows the repricing error analysis that validates the forward stub solution as the one able to minimize errors and improve the curve quality. As visible the excluding solution is the worst one and it is also unable to reprice correctly the excluded instruments. The other solutions instead are able to reprice perfectly the 2-months maturity OIS which is included in the overlapping solution but not in the Stub. The Root Mean Square Error (RMSE) and the Max Error (ME) shows that the best approach to solve the problem is by means of the Forward Stub that presents the lower values of RMSE and ME.
        \begin{figure}[!h]
        \centering
    ...
    \section{Mixed Interpolation}\label{section:3.2}
        The {\itshape Mixed Interpolation}, as already introduced before,  is a particular technique which merges two different kind of interpolation. Since the overnight curve time series shows a piecewise constant behaviour in the short term section, the best way to replicate this kind of shape is by means of a constant interpolation on discount factor which provides a stepped overnight curve. Otherwise, assuming flat forward rates on the mid-long term section can not be good and it becomes necessary to switch to another interpolation technique which provides smooth forwards as the monotone cubic one does. The correct point where doing the interpolation change can not be established consistently but the suggestion is to use a flat interpolator up to the end of the ECB dated OIS strip and then switch to the cubic one. The mixed interpolation is implemented in QuantLib 1.8 version in two different ways:
        ...
        \begin{figure}[!h]
        \centering
        \includegraphics[width=0.7\textwidth]{images/splitrange.png}
        ...
        As visible in the previous figure this approach produces a stepped overnight till the switch pillar and then a smooth curve due to the monotone cubic interpolation. 
        
        Finally, the table below summarize the repricing error analysis results.
        
        \begin{figure}[!h]
        \centering
        \includegraphics[width=0.9\textwidth]{images/repricingmixed.png}
        ...
        Analyzing the repricing table above it is possible to draw some conclusions. First of all, looking to the not included instruments, it is visible that the only approach that leads to higher errors is the Hyman Monotone Cubic one which fails to reprice correctly the 2-months maturity OIS (which is the instrument excluded in favor of the Stub). As already explained in the previous section the Stub implied quote is derived imposing a no-arbitrage condition and assuming a flat interpolation. Since the other approaches are interpolated linearly they re-price that instrument perfectly but this is not the Cubic Spline case that, in fact, leads to a relevant error of 0.97 basis points. Since a log-linear interpolation for the whole curve leads to big flat section on the mid-long term and since the monotone cubic one avoids the introduction of the forward stub, this work suggest to use the mixed interpolation because it links the benefits of a flat interpolator in the short term and the benefits of the smooth interpolator on the mid-long term.
