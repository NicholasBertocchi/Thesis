...
\end{figure}
    ...
    In fact, Figure \ref{fig:includingcurve} shows how the curve is pushed down till $-0.40\%$ and then turns back to the ECB OIS level. It is quite obvious that this strange curve's behaviour is not due to market quotes but to calibration's discrepancy resulting from the overlapping section. To solve this problem different approach can be followed:
     \begin{itemize}
        ...
        \label{fig:exclude}
        \end{figure}
         Looking to Figure \ref{fig:excludingcurve} is visible that the calibrator extrapolates backward from ECB Dated market quote the non-covered part of the curve leading to high distortions especially when preceding instrument and ECB OIS quotes are distant. 
        \begin{figure}[!h]
        \centering
        ...
        $t\ped{5} = \mbox{Second ECB dated OIS maturity date}$ \\
        ...
        Calibrating the overnight curve using this forward Stub improve consistently the curve quality avoiding unwanted distortion and oscillation as it will be shown later taking advantage of the repricing error analysis. Otherwise, this solution can be implemented only if the bootstrap algorithm uses a flat interpolation for the short-term section of the curve. Interpolating, for example, log-linearly the overnight curve becomes piecewise constant and this assumption permits to derive the $\int_{t\ped{2}}^{t\ped{3}} f(s)ds$ value that must be equal to ECB OIS market quote. It is also important to underlying that the Stub algorithm produces good results also in limit cases as, for example, when $\tau(t\ped{1},t\ped{2}) \longrightarrow \frac{1}{365}$ that represents the particular case in which the Stub quote duration is just 1 day. 
    ...
    \section{Mixed Interpolation}\label{section:3.2}
        ...
        \begin{figure}[!h]
        \centering
        \includegraphics[width=0.82\textwidth]{images/repricingmixed.png}
        \caption{Repricing error analysis using a linear interpolation for the whole curve ($1^{st}$ column), a monotonic cubic spline for the whole curve ($2^{nd}$ column), a share range mixed interpolation ($3^{rd}$ column) and a split range mixed interpolation ($4^{th}$ column).}
        \label{fig:repricingmixed}
        \end{figure}
        Analyzing the repricing table above it is possible to draw some conclusions. First of all it is visible that the only approach that leads to higher errors in the short section is the monotone cubic one which fails to reprice correctly the 2-months maturity OIS (which is the instrument excluded in favor of the  Forward Stub). As already explained in the previous section the Stub implied quote is derived imposing a no-arbitrage condition and assuming a flat interpolation. Since the other approaches are interpolated linearly they re-price that instrument perfectly but this is not the Cubic Spline case that, in fact, leads to a relevant error of 0.97 basis points. Since a log-linear interpolation for the whole curve leads to big flat section on the mid-long term and since the monotone cubic one avoids the introduction of the forward stub, this work suggest to use the mixed interpolation because it links the benefits of a flat interpolator in the short term and the benefits of the smooth interpolator on the mid-long term.
        
    \section{Jump estimation} \label{section:3.3}
    As already discussed in previous chapters, the key point in a state-of-the-art curve bootstrapping is to obtain smooth forward rates. In order to do that, for even the best interpolation scheme to be effective, any market jump must be removed before the curve calibration and than added back at the end of the process. To perform the analysis for the EUR market case the same dataset has been used while for the USD case another dataset as of October 5, 2016 (spot date October 7, 2016), has been used which is shown in the following figure.
...
To estimate jumps sizes Ametrano and Mazzocchi (\cite{amemaz}) propose a four-step approach similar to the one used by Burghardt (\cite{burgh}):
       \begin{enumerate}
        ...
        Figures \ref{fig:firstjumpeur1}, \ref{fig:firstjumpeur2} and \ref{fig:firstjumpeur3} shows the impact of first and second jump inclusion in the curves shape. As visible, the short term section of the curve is changed due the fact that the first jumps has been accounted. To preserve the value of the instantaneous forward rate integral in that section, the calibration algorithm pushes down all the segment changing the curve shape.
        \begin{figure}[!h]
        \centering
        ...
        \begin{figure}[!h]
        \centering
        \includegraphics[width=0.8\textwidth]{images/firstjumpeur3.png}
...
This behaviour tends to be constant during time, in fact, in the last two years, the Fed Funds time series shows negative jumps every end-of-month with no exceptions. To estimate this kind of jumps, however, the Ametrano-Mazzocchi approach for the EUR market case must be reviewed so that it can account negative sizes. In particular the formula at point 2 becomes:
        ...
        \begin{figure}[!h]
        \centering
        \includegraphics[width=0.75\textwidth]{images/firstjumpusd1.png}
        \caption{First two year section of the USD overnight curve without jumps.}
        \label{fig:firstjumpusd1}
        \end{figure}
        \begin{figure}[!h]
        \centering
        \includegraphics[width=0.75\textwidth]{images/firstjumpusd3.png}
        \caption{The same curve in Figure \ref{fig:firstjumpusd1} including first and second jumps.}
