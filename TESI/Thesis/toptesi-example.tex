...
\chapter{Improving Overnight Curve} \label{chap:3}
    ...
    In order to do that, for even the best interpolation scheme to be effective, any market rate must be removed before the curve calibration and than added back at the end of the process.
    
       \subsection{The EUR market case: Positive Jumps} \label{subsection:3.3.2}
       A rate jump can be seen, in a financial point of view, as a higher index fixing due to increased search of liquidity of market participants caused by end-of-month or end-of-year capital requirements. This definition is good for the EUR case in which market evidence shows positive jumps. 
       ...
       \end{figure}
       However, it is not consistent for the U.S. case in which market evidence shows negative jumps and so American markets participants tend to have a decreasing necessity of liquidity every end-of-month. The most relevant rate jump is the so called {\it Turn-Of-Year} (TOY) effect observable in the last working day in market quotations spanning across the end of the year. Other Euribor indexes with longer tenor display smaller jumps when their maturity crosses the same border, in fact it is visible that the jumps amplitude decrease as the rate tenor increase.\\
       To estimate jumps sizes Ametrano and Mazzocchi (\cite{amemaz}) propose an approach similar to the one used by Burghardt (\cite{burgh}):
       \begin{enumerate}
           \item The first step is to built an overnight curve calibrating by means of a linear/flat interpolation and including all liquid market quotes available (high pillar density improve the analysis); 
        ...
        \begin{figure}[!h]
        \centering
        \includegraphics[width=0.8\textwidth]{images/usdcurvenojump.png}
        \caption{First two year section of the USD overnight curve without jumps.}
        \label{fig:usdcurvenojump}
        \end{figure}
    
        \begin{figure}[!h]
        \centering
        \includegraphics[width=0.8\textwidth]{images/usdcurvejump.png}
        \caption{The different shape caused by first jump inclusion.}
        \label{fig:usdcurvejump}
        \end{figure}
