...
\end{figure}
    ...
    \section{Mixed Interpolation}\label{section:3.2}
       ...
       Since using a log-linear or a monotonic cubic natural spline interpolation for the whole curve leads, in both cases, to distortions, this work suggest that the best way to model the overnight curve behaviour is to merge this two techniques leading to a mixed interpolator. The mixed interpolation is implemented in QuantLib 1.8 version in two different ways:
        ...
        The aim of this analysis is to demonstrate that the mixed interpolation is the best way to interpolate the overnight curve looking to the out-of-curve OIS repricing errors. In order to do that, a comparison between a all Log-Linear Curve, a all Hyman Natural Spline curve, a Mixed Log-linear/Hyman share range curve and a Mixed Log-linear/Hyman split range curve has been shown in Figure \ref{fig:repricingmixed}. The repricing error analysis indicates that the four methods produce almost the same results in the short term section curve except for the Hyman Monotone Cubic Spline that fails to reprice the 2-month maturity OIS which is the one excluded in favor of the forward stub presented in the previous section. As already explain, the necessary condition that ensures a perfect reprice of the overlapping instruments is that a piecewise constant interpolation is used. Going further, since after 15 months all the instruments are included a different approach has been followed. In fact the red numbers have been discarded singularly from the bootstrap and then repriced do understand how the four different curve works on mid/log term. Since the last three are Hyman Cubic Splines after OIS15M pillar they produces similar results as expected while the first one, which is whole Log-Linear, fail to reprice this mid/lond tenor instruments especially in the 5Y-6Y tenor, where the curve tends to be very steep. \\
        Summing up, the analysis shows how a whole Log-Linear curve fails to reprice mid/long tenor OIS and a whole Hyman Monotone Cubic Spline curve does not ensure the perfect overlapping instruments reprice; which means that including the forward stub produces distortions. The only methods which combines the benefits of the preceding approach is the Mixed Interpolation.\\
